...
def play(hh, mm, ss):
    gui2 = Toplevel(gui)
    wake = Label(gui2, text="It's Time Buddy", fg="red", font=("Arial", 20)).place(
        x=0, y=50
    )
    exit_button = Button(
        gui2, text="STOP", command=lambda: (stop(), gui2.destroy())
    ).place(x=70, y=100)
    h = str(hh)
    m = str(mm + 5)
    s = str(ss)
    snooze = Button(
        gui2,
        text="SNOOZE",
        command=lambda: (thread_run(h, m, s), stop(), gui2.destroy()),
    ).place(x=70, y=150)
    pygame.mixer.init()
...
def alarm(h, m, s):
    ...
    for i in range(len(hours)):
        while True:
            if (
                hours[i] == datetime.now().hour
                and minute[i] == datetime.now().minute
                and second[i] == datetime.now().second
            ):
                play(hh, mm, ss)
                break
...
text = Label(
    gui, text="Set Your Alarm:", bg="#2F2D2D", fg="white", font=("Arial", 16, "bold")
)
text1 = Label(gui, bg="#2F2D2D", fg="white", font=("Arial", 16, "bold"))
text2 = Label(
    gui, text="Saved Alarms:", bg="#2F2D2D", fg="white", font=("Arial", 16, "bold")
)
hour = StringVar()
min = StringVar()
sec = StringVar()
...
submit = Button(
    ...
    text="Set Alarm",
    command=lambda: (
        thread_run(hourTime.get(), minTime.get(), secTime.get()),
        clearTxt(),
    ),
).place(x=220, y=260)
