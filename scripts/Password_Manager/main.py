...
def Print_Passwd(site):
    ...
    try:
        while True:
            x = pickle.load(file)
            z = str(base64.b64decode(x['site'].decode("utf-8"))).lstrip("b'").rstrip("'")
            if site == z:
                user = str(base64.b64decode(x['user'].decode("utf-8"))).lstrip("b'").rstrip("'")
                passwd = str(base64.b64decode(x['passwd'].decode("utf-8"))).lstrip("b'").rstrip("'")
                return user, passwd
    except:
        pass
